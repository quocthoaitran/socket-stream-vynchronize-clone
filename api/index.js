...
io.on("connection", (socket) => {
  ...
  console.log(`The id ${socket.id} was connected. Connections: ${connections.length} sockets connected`);
  ...
  socket.on("new_user", (data, callback) => {
    ...
    if (init) {
      ...
      // Host username
      io.sockets.adapter.rooms.get(`room-${socket.roomID}`).hostName = socket.username;
    ...
    // Set host label
    updateHostRoom(socket.roomID, io.sockets.adapter.rooms.get(`room-${socket.roomID}`).hostName)

    // Update Queue video
    updateQueueVideos(socket.roomID)

    // Gets current video from room variable
    var currVideo = io.sockets.adapter.rooms.get(`room-${socket.roomID}`).currVideo
  ...
  socket.on('check_connection', callback => {
    ...
    else if (socket.id === io.sockets.adapter.rooms.get(`room-${socket.roomID}`).host) {
        callback({
          connected: true,
          isHost: true
        })
  ...
  socket.on('send_message', data => {
    io.sockets.in(`room-${socket.roomID}`).emit('new_message', { message: data, user: socket.username})
  })

  socket.on('sync_video', data => {
    io.sockets.adapter.rooms.get(`room-${socket.roomID}`).currVideo = {...io.sockets.adapter.rooms.get(`room-${socket.roomID}`).currVideo, ...data}
  ...
  socket.on('delete_queue_item', data => {
    if (socket.id === io.sockets.adapter.rooms.get(`room-${socket.roomID}`).host) {
      io.sockets.adapter.rooms.get(`room-${socket.roomID}`).queue.splice(data, 1)
      updateQueueVideos(socket.roomID)
    }
  })
  ...
  socket.on('play_video', data => {
    if (socket.id === io.sockets.adapter.rooms.get(`room-${socket.roomID}`).host) {
      ...
      io.sockets.adapter.rooms.get(`room-${socket.roomID}`).queue.splice(data.index, 1)
      updateQueueVideos(socket.roomID)
      updateCurrVideo(socket.roomID)
    }
  })
  ...
  socket.on('disconnect', () => {
    ...
    console.log(`${socket.id} disconnected. ${connections.length} sockets connected`)
  ...
  socket.on('request_to_change_host', () => {
    io.to(io.sockets.adapter.rooms.get(`room-${socket.roomID}`).host).emit('confirm_request', {id: socket.id, username: socket.username})
  })

  socket.on('host_confirm_request', data => {
    if (data.isConfirm) {
      io.sockets.adapter.rooms.get(`room-${socket.roomID}`).host = data.id;
      io.sockets.adapter.rooms.get(`room-${socket.roomID}`).hostName = data.username;
  ...
  function updateCurrVideo(roomID) {
    io.sockets.in(`room-${roomID}`).emit('change_video_client', io.sockets.adapter.rooms.get(`room-${roomID}`).currVideo)
  }

  // Update the room usernames
  function updateRoomUsers(roomID) {
    if (io.sockets.adapter.rooms.get(`room-${roomID}`) !== undefined) {
      var roomUsers = io.sockets.adapter.rooms.get(`room-${roomID}`)
